...
export const getJson = async <TResponse>(path: string, fallbackMessage: string): Promise<TResponse> => {
  const response = await fetch(buildApiUrl(path));

  if (!response.ok) {
    throw new Error(await parseErrorMessage(response, `${fallbackMessage} (${response.status})`));
  }
