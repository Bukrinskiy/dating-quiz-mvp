...
export const getTrackingAttribution = (search: string): TrackingAttribution => {
  const params = getTrackingParams(search);
  const tokens = (window as Window & { tokens?: { clickid?: string; bcid?: string } }).tokens;
  const clickId = tokens?.clickid || getCookieValue("clickid") || params.get("clickid");
  const bcid = tokens?.bcid || getCookieValue("bcid") || params.get("bcid");
...
export const getClickId = (search: string): string | null => {
  const attribution = getTrackingAttribution(search);
  return attribution.clickId || attribution.bcid;
};

export const addClickIdToPath = (path: string, search: string): string => {
  const { params } = getTrackingAttribution(search);
