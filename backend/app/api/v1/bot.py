...
from app.schemas.bot import (
    ...
    BotAdminAccessCodeCreateRequest,
    BotAdminAccessCodeCreateResponse,
...
@router.post(
    ...
    response_model=BotAdminAccessCodeCreateResponse,
    dependencies=[Depends(_require_internal_token)],
)
def bot_admin_access_code_create(
    payload: BotAdminAccessCodeCreateRequest,
    db: Session = Depends(get_db),
) -> BotAdminAccessCodeCreateResponse:
    access_code = AccessCodeService(db).create_code(
        admin_telegram_user_id=payload.admin_telegram_user_id,
        admin_telegram_username=payload.admin_telegram_username,
        expires_at=payload.expires_at,
    )
    return BotAdminAccessCodeCreateResponse(
        status="created",
